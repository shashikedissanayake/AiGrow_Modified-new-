...
{
    public class DL_BayDevice
    {
        public bool insert(Model.ML_BayDevice device)
        {
            var para = new MySqlParameter[7];
            para[0] = new MySqlParameter("@device_unique_id", device.bay_device_unique_id);
            para[1] = new MySqlParameter("@device_name", device.bay_device_name);
            para[2] = new MySqlParameter("@device_type", device.device_type);
            para[3] = new MySqlParameter("@io_type", device.io_type);
            para[4] = new MySqlParameter("@bay_id", device.bay_id);
        ...
        public bool doesDeviceExist(string device)
        {
            ...
            int count = MySQLHelper.ExecuteDataTable(DBConnection.connectionString, System.Data.CommandType.Text, "SELECT * FROM `bay_device` WHERE bay_device_unique_id = @device_id", para).Rows.Count;
            return count >= 1;
        }
