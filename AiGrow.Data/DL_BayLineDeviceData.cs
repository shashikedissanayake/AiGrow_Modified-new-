...
namespace AiGrow.Data
{
    public class DL_BayLineDeviceData
    {
        public bool insert(Model.ML_BayLineDeviceData data)
        {
            ...
            return MySQLHelper.ExecuteNonQuery(DBConnection.connectionString, CommandType.Text, "INSERT INTO bay_line_device_data (device_unique_id, received_server_time, collected_time, data, data_unit) VALUES (@unique_id, NOW(), @time, @data, @unit)", para) != -1;
        }
        ...
        public DataTable selectDataSet(string device,string from,string to)
        {
                ...
                return MySQLHelper.ExecuteDataTable(DBConnection.connectionString, System.Data.CommandType.Text, "SELECT bldd.collected_time, bldd.data FROM bay_line_device_data bldd WHERE bldd.device_unique_id = @device_id AND (bldd.collected_time BETWEEN @fromDate AND @toDate)", para);
